...
ggsave(here("local_data","plot_kraje2.svg"), plot_kraje2)
plot_kraje2
```
...
ggsave(here("local_data", "hist_vek.svg"), hist_vek)
...
hist_vek_podil <- cela_data %>% filter(!is.na(ended.hlavni), age >= 15, age <= 26) %>%
  ...
  scale_y_continuous("Podíl zúčastnění/registrovaní", breaks = age_breaks, labels = paste0(age_breaks, "%"))

hist_vek_podil
ggsave(here("local_data","hist_vek_podil.svg"))
