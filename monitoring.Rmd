...
odpovedi_v_krajich <- cela_data %>% filter(!is.na(kraj), kraj != 15, age >= 15, age <= 26) %>%
...
pie_sex <- df_sex %>% ungroup() %>% filter(dokonceno) %>%
  ...
  ggplot(aes(x = "", y = n, fill = sex)) + geom_bar(stat = "identity") + coord_polar("y", start = 0) + scale_fill_manual("Pohlaví", values = c("MUŽI" = "white", "ŽENY" = darkest_fill, "NEUVEDENO" = midd_fill)) + 
  geom_text(aes(y = 2*n/3 + c(0, cumsum(n)[-length(n)]), 
            label = paste0(round(100 * n/sum(n)), "%"), color = sex == "MUŽI"), x = 1, size=5, family = "Robot", fontface = "bold")+
  scale_color_manual(values = c(dark_blue_color,"white"), guide = FALSE) +
  theme(axis.text = element_blank(), axis.ticks = element_blank(), axis.line = element_blank(),
        axis.title = element_blank())
