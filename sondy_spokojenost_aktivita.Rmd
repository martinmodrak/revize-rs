...
effects_by_prediction_spokojenost <- function(var_name, var_values = levels(spokojenost_pro_model[[var_name]]), var_labels = var_values, order = FALSE, n_samples = 500, max_lines = 200) {
  ...
  effects_by_prediction(fit_spokojenost, 
                        ...
                        n_samples = n_samples,
                        max_lines = max_lines) + ggtitle(paste0("Spokojenost dle ", var_name))
}
```
...
effects_by_prediction_aktivita <- function(var_name, var_values = levels(aktivita_pro_model[[var_name]]), var_labels = var_values, order = FALSE) {
  ...
  effects_by_prediction(fit_aktivita, 
                        ...
                        response_title = "P(spíše aktivní než ne)") + ggtitle(paste0("Aktivita dle ", var_name))
}
```

```{r}
combined_effects_by_prediction <- function(var_name, var_values = levels(aktivita_pro_model[[var_name]]), var_labels = var_values, order = FALSE) {
  effects_by_prediction_spokojenost(var_name, var_values, var_labels, order) %>% print()
  effects_by_prediction_aktivita(var_name, var_values, var_labels, order) %>% print()
}
```
...
```{r}
combined_effects_by_prediction("velikost_obce")
