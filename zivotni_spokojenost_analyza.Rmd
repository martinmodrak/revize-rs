...
visualizuj_skupinu <- function(var, which_visualize) {
  df <- cela_data %>%
    select(!!var, !!which_visualize) %>% 
    pivot_longer(cols = !!which_visualize,names_to = "promenna", values_to = "hodnota") %>% 
    group_by(pomaha_skodi_skupina, promenna) %>% 
...
cl <- hdbscan(pomaha_skodi, minPts = 5)
  cl
#cela_data %>% group_by(pomaha_skodi_skupina) %>% summarize_each(vars(co_pomaha_roveringu = mean(co_pomaha_roveringu))
```
