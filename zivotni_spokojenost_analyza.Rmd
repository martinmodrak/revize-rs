...
cela_data %>% 
  mutate(frekvence_kratkych_akci = factor(frekvence_kratkych_akci, levels = c("nikdy","rocne","nekolik_rocne","mesicne","nekolik_mesicne","tydne","nekolik_tydne"))) %>% 
  group_by(frekvence_kratkych_akci) %>% 
  summarize(spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm=T)) %>% 
  ggplot(aes(x=frekvence_kratkych_akci,y=spokojenost_s_roverskym_programem),cela_data) + geom_bar(stat = "identity")
...
p2 <- cela_data %>% 
  ...
  group_by(frekvence_vicedennich_akci) %>% 
  summarize(spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm=T)) %>% 
  ggplot(aes(x=frekvence_vicedennich_akci,y=spokojenost_s_roverskym_programem),cela_data) + geom_bar(stat = "identity")
p1/p2
...
p2 <- cela_data %>% 
  ...
  group_by(frekvence_velkych_akci) %>% 
  summarize(spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm=T)) %>% 
  ggplot(aes(x=frekvence_velkych_akci,y=spokojenost_s_roverskym_programem),cela_data) + geom_bar(stat = "identity")
p1/p2
...
lss_pro_kategorie(quo(kategorie_respondenta)) %>% ggplot(aes(x = as.character(kategorie_respondenta), y =spokojenost_s_roverskym_programem  )) + geom_bar(stat="identity") + xlab("kategorie respondenta") +   theme(aspect.ratio = 0.5) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

lss_pro_kategorie(quo(role_skauting_vedouci_roveru)) %>% ggplot(aes(x = as.character(role_skauting_vedouci_roveru), y =lss)) + geom_bar(stat="identity") + xlab("role skauting") +   theme(aspect.ratio = 0.5) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
lss_pro_kategorie(quo(zivotni_faze)) %>% 
  ...
  ggplot(aes(x = as.character(zivotni_faze), y =spokojenost_s_roverskym_programem  )) + geom_bar(stat="identity") + xlab("zivotni_faze") +   
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
lss_pro_kategorie(quo(kraj)) %>% 
  mutate(zivotni_faze = recode(as.character(zivotni_faze), `1`="Studuji",`2`="Pracuji",`3`="Studuji i pracuji",`4`="Nepracuji ani nestuduji")) %>% 
  ggplot(aes(x = as.character(kraj), y =spokojenost_s_roverskym_programem  )) + geom_bar(stat="identity") + xlab("zivotni_faze") +   
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
kraj_val <- lss_pro_kategorie(quo(kraj)) %>% 
  arrange(spokojenost_s_roverskym_programem) %>% 
  mutate(kraj = haven::as_factor(kraj) %>% as.character()) %>% pull(kraj)
...
lss_pro_kategorie(quo(kraj)) %>% 
  ...
  mutate(kraj = haven::as_factor(kraj) %>% as.character()) %>% 
  mutate(kraj = factor(kraj, levels = kraj_val)) %>% 
  ggplot(aes(x = kraj, y =spokojenost_s_roverskym_programem  )) + geom_bar(stat="identity") + xlab("kraj")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

lss_pro_kategorie(quo(sex)) %>% 
  mutate(sex = haven::as_factor(sex) %>% as.character()) %>% 
  ggplot(aes(x = sex, y =spokojenost_s_roverskym_programem  )) + geom_bar(stat="identity") + xlab("pohlavi")+theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
cela_data %>% 
  group_by(n_roli) %>% 
  summarize(spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm = T), n =n()) %>% 
ggplot(aes(x=n_roli, y = spokojenost_s_roverskym_programem, size = n)) + geom_point()
...
cela_data %>% 
  group_by(n_sluzba) %>% 
  summarize(spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm = T), n =n()) %>% 
ggplot(aes(x=n_sluzba, y = spokojenost_s_roverskym_programem, size = n)) + geom_point()
...
cela_data %>% 
  group_by(n_s_cim_spokojen) %>% 
  summarize(spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm = T), n =n()) %>% 
ggplot(aes(x=n_s_cim_spokojen, y = spokojenost_s_roverskym_programem, size = n)) + geom_point()
...
cela_data %>% 
  group_by(n_s_cim_nespokojen) %>% 
  summarize(spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm = T), n =n()) %>% 
ggplot(aes(x=n_s_cim_nespokojen, y = spokojenost_s_roverskym_programem, size = n)) + geom_point()
...
cela_data %>% 
  ...
  summarize(lss = mean(lss, na.rm =T),
            spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm =T),
            n = n()) %>% 
  ggplot(aes(x=role_skauting_vedouci_roveru, y = spokojenost_s_roverskym_programem, size = n)) + geom_bar(stat = "identity")
...
cela_data %>% 
  ...
  summarize(lss = mean(lss, na.rm =T),
            spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem, na.rm =T),
            n = n()) %>% 
  ggplot(aes(x=role_skauting_vedouci_roveru, y = lss, size = n)) + geom_bar(stat = "identity")
...
lss_cor <- expanded %>% select(session, lss, spokojenost_s_roverskym_programem,kategorie_kompetence, kompetence_odpoved, kompetence) %>% filter(kategorie_kompetence %in% c("dulezite","zvladam","rozvijim","skauting")) %>%
  pivot_wider(id_cols = c("session", "kompetence", "lss","spokojenost_s_roverskym_programem"), names_from = kategorie_kompetence, values_from = kompetence_odpoved,values_fn = list(kompetence_odpoved = mean)) %>% 
  mutate(dulezite = unlist(dulezite))
  group_by(session) %>% 
  summarize(lss = mean(lss), 
            spokojenost_s_roverskym_programem = mean(spokojenost_s_roverskym_programem), 
...
m <- lss_cor %>% 
  select(lss, spokojenost_s_roverskym_programem,dulezite, zvladam, rozvijim, skauting) %>% 
...
cela_data %>% 
  filter(sex!=3) %>% 
  mutate(sex = haven::as_factor(sex)) %>%
  select(sex,starts_with("role_skauting_")) %>% 
  pivot_longer(cols = starts_with("role_skauting_"), values_to = "procenta", names_to = "role") %>% 
...
cela_data %>% 
  filter(sex!=3) %>% 
  mutate(sex = haven::as_factor(sex)) %>%
  select(sex,starts_with("vyroky_o_roveringu_"),-starts_with("vyroky_o_roveringu_stredisko"),-vyroky_o_roveringu_zazil,-vyroky_o_roveringu_zazil_2) %>% 
  pivot_longer(cols = starts_with("vyroky_o_roveringu_"), values_to = "procenta", names_to = "vyroky_o_roveringu") %>% 
  mutate(vyroky_o_roveringu = str_remove(vyroky_o_roveringu,"vyroky_o_roveringu_zazil_") %>% str_remove("2_"),
         procenta = as.numeric(procenta)) %>% 
...
cela_data %>% 
  filter(sex!=3) %>% 
  mutate(sex = haven::as_factor(sex)) %>%
  select(sex,starts_with("problemy_roveringu_"),-starts_with("problemy_roveringu_stredisko")) %>% 
  pivot_longer(cols = starts_with("problemy_roveringu_"), values_to = "procenta", names_to = "problemy_roveringu") %>% 
  mutate(problemy_roveringu = str_remove(problemy_roveringu,"problemy_roveringu_"),
         procenta = as.numeric(procenta)) %>% 
...
cela_data %>% 
  select(sex,starts_with("fungovani_skautskeho_oddilu_")) %>% 
  pivot_longer(cols = starts_with("fungovani_skautskeho_oddilu_"), values_to = "procenta", names_to = "fungovani_skautskeho_oddilu") %>% 
  mutate(fungovani_skautskeho_oddilu = str_remove(fungovani_skautskeho_oddilu,"fungovani_skautskeho_oddilu_"),
         procenta = as.numeric(procenta)) %>% 
...
```{r}
cela_data %>%
  mutate(pocet_clenu_spolecenstvi=haven::as_factor(pocet_clenu_spolecenstvi)) %>% 
  mutate(pocet_clenu_spolecenstvi = factor(pocet_clenu_spolecenstvi, levels = c("5 a méně", "6–10","11–20","21–30","31 a více"))) %>% 
  select(pocet_clenu_spolecenstvi,starts_with("fungovani_skautskeho_oddilu_")) %>% 
  pivot_longer(cols = starts_with("fungovani_skautskeho_oddilu_"), values_to = "procenta", names_to = "fungovani_skautskeho_oddilu") %>% 
  mutate(fungovani_skautskeho_oddilu = str_remove(fungovani_skautskeho_oddilu,"fungovani_skautskeho_oddilu_"),
         procenta = as.numeric(procenta)) %>% 
...
cela_data %>% 
  ...
  group_by(pocet_clenu_spolecenstvi) %>% 
  summarize(lss = mean(lss, na.rm = T),
            spoko_rp = mean(spokojenost_s_roverskym_programem),
            n=n())
...
cela_data %>%
  mutate(pocet_clenu_spolecenstvi=haven::as_factor(pocet_clenu_spolecenstvi)) %>% 
  mutate(pocet_clenu_spolecenstvi = factor(pocet_clenu_spolecenstvi, levels = c("5 a méně", "6–10","11–20","21–30","31 a více"))) %>% 
  select(pocet_clenu_spolecenstvi,starts_with("vyroky_o_roveringu_"),-starts_with("vyroky_o_roveringu_stredisko"),-vyroky_o_roveringu_zazil,-vyroky_o_roveringu_zazil_2) %>% 
  pivot_longer(cols = starts_with("vyroky_o_roveringu_"), values_to = "procenta", names_to = "vyroky_o_roveringu") %>% 
  mutate(vyroky_o_roveringu = str_remove(vyroky_o_roveringu,"vyroky_o_roveringu_"),
         procenta = as.numeric(procenta)) %>% 
