...
cela_data_role <- cela_data %>% rozsir_vsechna_mc()
cela_data_role %>% select(starts_with("role_skauting_"),lss) %>% cor(use = "complete.obs") %>% knitr::kable(digits = 2)
...
cela_data_role %>% select(starts_with("role_skauting_")) %>% cor(use = "complete.obs") %>% round(2) %>% knitr::kable(digits = 2)
...
```{r factor analysis}
role_fa <- cela_data_role %>% select(starts_with("role_skauting_")) 
...
cr <- cela_data_role %>% select(starts_with("vychovne_nastroje_"),lss, spokojenost_s_roverskym_programem, spokojenost_s_fungovanim_v_junaku) %>% as.matrix() %>% Hmisc::rcorr() 
