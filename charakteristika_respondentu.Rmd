...
toc-title: "Obsah"
abstract: "Tento dokument podává základní přehled o tom, kdo nám na otázky odpovídal."
output:
  word_document:
    toc: yes
...
```{r}
plots$vek <- datasety_wide$pouzitelne %>% group_by(age) %>% summarise(pocet = n()) %>% mutate(podil = pocet / sum(pocet)) %>%
...
compute_reprezentativnost_data <- function(hlavni_data, registrace_strediska, cols) {
  ...
  skautis_seskupeno <- registrace_strediska %>% 
    group_by_at(all_of(cols)) %>%
    summarise(pocet_skautis = sum(pocet_clenu)) %>%
    ungroup()
...
# 
# 
# TODO: porovnání pohlaví napřímo je problematické kvůli rozdílnému věkovému složení!
```
...
```{r}
hlavni_data %>% group_by(sex) %>% summarise(prumer_vek = mean(age)) %>% table_format()
```
...
```{r}
hlavni_data %>% group_by(reg_c_strediska) %>% summarise(pocet = n(), nazev = nazev_strediska[1]) %>% filter(!is.na(reg_c_strediska), pocet > 8) %>% arrange(desc(pocet)) %>% table_format
```
