...
registrace_strediska <- nacti_skautis_pocty_clenu(here("public_data/pocet-clenu-strediska-2019.csv")) %>% filter(Year == 2019, ID_UnitType == "stredisko") %>% inner_join(strediska_kraje, by = c("RegistrationNumber")) %>%
  transmute(kraj = uprav_nazvy_kraju(UnitName_kraj), 
...
compute_reprezentativnost_data <- function(hlavni_data, registrace_strediska, cols) {
  ...
  if("kraj" %in% cols) {
    respondenti_seskupeno <- respondenti_seskupeno %>%
      filter(!is.na(kraj), kraj != "Nevím/Nejsem součástí žádného střediska")
  }
  ...
  reprezentativnost_data <- respondenti_seskupeno %>% 
    inner_join(skautis_seskupeno, by = cols) %>%
    mutate(podil_respondentu = pocet_respondentu / sum(pocet_respondentu),
...
plot_reprezentativnost(reprezentativnost_data_vse %>% filter(vekova_skupina == "nad_18"), facet_wrap(~ pocet_clenu_strediska)) + plot_annotation("Reprezentativnost nad 18", "dle kraje a velikosti střediska")

plot_reprezentativnost(reprezentativnost_data_vse %>% filter(vekova_skupina == "do_18"),  facet_wrap(~ pocet_clenu_strediska))  + plot_annotation("Reprezentativnost nad 18", "dle kraje a velikosti střediska")
...
compute_reprezentativnost_data(hlavni_data, registrace_strediska, cols = c("pocet_clenu_strediska","kraj")) %>%
  plot_reprezentativnost(facet_wrap( ~ pocet_clenu_strediska)) + plot_annotation("Reprezentativnost", "dle kraje, velikosti střediska")
```
