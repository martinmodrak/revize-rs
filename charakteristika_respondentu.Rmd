...
plot_reprezentativnost <- function(reprezentativnost_data, facet = NULL, x = kraj) {
  reprezentativnost_data %>% 
    ...
    rename(podil_SkautIS = podil_skautis, `podil_Průzkum` = podil_respondentu) %>%
...
plots$reprezentativnost_kraj_stredisko_nad18 <- plot_reprezentativnost(reprezentativnost_data_vse %>% filter(vekova_skupina == "nad_18"), facet_wrap(~ pocet_clenu_strediska)) + plot_annotation("Reprezentativnost nad 18", "dle kraje a velikosti střediska")

plots$reprezentativnost_kraj_stredisko_nad18

plots$reprezentativnost_kraj_stredisko_do18 <- plot_reprezentativnost(reprezentativnost_data_vse %>% filter(vekova_skupina == "do_18"),  facet_wrap(~ pocet_clenu_strediska))  + plot_annotation("Reprezentativnost nad 18", "dle kraje a velikosti střediska")
...
```{r}
hlavni_data %>% group_by(reg_c_strediska) %>% summarise(pocet = n(), nazev = nazev_strediska[1]) %>% arrange(desc(pocet))
```
