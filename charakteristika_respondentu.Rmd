...
```{r results='asis'}
if(gitbook_output) {
  cat("# (PART\\*) Výsledky průzkumu {-}\n")
} else {
  cat("# Výsledky průzkumu {-}\n")
}
```
...
což činí grafy méně čitelnými - a zároveň skutečně o této skupině nemůžeme v porovnání s ostatními kvůli tomuto malému počtu říct nic s jistotou.
...
A jedna z hlavních otázek, kterou charakterizujeme
