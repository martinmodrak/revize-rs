...
strediska_kraje %>% filter(RegistrationNumber == "625.06")

nrow_before <- nrow(hlavni_data)
#respondenti_seskupeno <- 
hlavni_data %>% 
  left_join(strediska_kraje, by = c("reg_c_strediska" = "RegistrationNumber")) %>%
  filter("kraj_nazev" != "UnitName_kraj") %>%
  select("reg_c_strediska", "kraj_nazev", "UnitName_kraj") %>%
  distinct()


registrace_strediska <- nacti_skautis_pocty_clenu(here("public_data/pocet-clenu-strediska-2019.csv")) %>% filter(Year == 2019) %>% mutate(RegVOJ = gsub("\\.[0-9]{2}$", "", RegistrationNumber))

registrace_VOJ <- nacti_skautis_pocty_clenu(here("public_data/pocet-clenu-VOJ.csv")) %>% filter(Year == 2019, ID_UnitType == "kraj" | ID_UnitType == "okres") %>% select(RegistrationNumber, UnitName)

registrace_strediska_voj <- registrace_strediska


head(hlavni_data$kraj_nazev)
```
