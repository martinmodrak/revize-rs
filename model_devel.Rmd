...
hist(diag(cov(mult_res)) - b_sd)
mean(diag(cov(mult_res)) - b_sd)
mvn_res <- t(rmvnorm(N_fixed, sigma = corr_matrix)) %*% diag(b_sd)
mean(diag(cov(mvn_res)) - b_sd)
...
ncat = 4
repeat {
  test_data <- simulate_data(N = 200, N_questions = 3, N_fixed = 5, N_random_groups = c(2,4), ncat = ncat, N_monotonic = 1)
...
set.seed(6847567)
sbc_steps <- 500
sbc_results <- sbc(model, function() { simulate_data(N = 100, N_questions = 3, N_fixed = 2, ncat = 4, N_random_groups = c(2,4),  N_monotonic = 1)}, N_steps = sbc_steps, control = list(adapt_delta = 0.1))
