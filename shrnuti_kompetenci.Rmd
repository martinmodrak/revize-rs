...
```{r}
hlavni_data_long %>% filter(kategorie_kompetence == "zvladam") %>% 
  group_by(kompetence) %>% 
  summarise(pocet_nezvladam = sum(kompetence_odpoved <= 3), podil_nezvladam = scales::percent(mean(kompetence_odpoved <= 3))) %>%
...
```{r}
hlavni_data_long %>% filter(kategorie_kompetence == "dulezite") %>% 
  group_by(kompetence) %>% 
  summarise(pocet_nedulezite = sum(kompetence_odpoved <= 3), podil_nedulezite = scales::percent(mean(kompetence_odpoved <= 3))) %>%
...
```{r}
hlavni_data_long %>% filter(kategorie_kompetence == "rozvijim") %>% 
  group_by(kompetence) %>% 
  summarise(pocet_nerozvijim = sum(kompetence_odpoved <= 3), podil_nerozvijim = scales::percent(mean(kompetence_odpoved <= 3))) %>%
...
```{r}
hlavni_data_long %>% filter(kategorie_kompetence == "skauting") %>% 
  group_by(kompetence) %>% 
  summarise(pocet_neskauting = sum(kompetence_odpoved <= 3), podil_neskauting = scales::percent(mean(kompetence_odpoved <= 3))) %>%
...
plot_kompetence_by_smooth(hlavni_data_long, "dulezite", age)
```
...
plot_kompetence_by_smooth(hlavni_data_long, "zvladam", age)
```
...
plot_kompetence_by_smooth(hlavni_data_long, "rozvijim", age)
```
...
plot_kompetence_by_smooth(hlavni_data_long, "skauting", age)
```
...
tabulka_rozdilu <- function(data_rozdil) {
  ...
  data_rozdil$data %>%
    ...
    arrange(desc(prumer)) %>%
    mutate(prumer = formatter(prumer)) %>%
    rename(!!nazev_prumer := prumer)
}
...
```{r}
hlavni_data_long %>% spocitej_data_rozdil(dulezite, rozvijim, "kompetence_pozitivni") %>% tabulka_rozdilu()
```
...
```{r}
hlavni_data_long %>% spocitej_data_rozdil(dulezite, skauting, "kompetence_pozitivni") %>% tabulka_rozdilu()
...
rozdil_rozvijim_skauting %>%
  ggplot(aes(rozdil_rozvijim_skauting)) +
    ...
    geom_vline(xintercept = 0, color = revize_cols("orange")) + vodorovne_popisky_x
```
...
```{r}
save_list_of_plots(plots,"plots_shrnuti_kompetenci")
```
