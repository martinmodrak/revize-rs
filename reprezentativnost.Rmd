...
reprezentativnost_vek <- function(data_skautis_, data_pruzkum_, pruzkum_nazev, min_vek = 15, max_vek = 21) {
  dle_veku_pruzkum <- data_pruzkum_ %>% 
    filter(!is.na(vek), vek >= min_vek, vek <= max_vek) %>%
    group_by(vek) %>%
    summarise(clenu = length(vek)) %>% ungroup() %>% mutate(zdroj = pruzkum_nazev)
...
reprezentativnost_vek(data_skautis_jednotlivci %>% filter(Year == 2018), data_kmen, "kmen")
reprezentativnost_vek(data_skautis_jednotlivci %>% filter(Year == 2015), data_sondy, "_sondy")
```
