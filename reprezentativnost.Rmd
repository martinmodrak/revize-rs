...
dle_veku_a_obce <- dle_veku_a_obce %>%
  ...
  mutate(procent = clenu / celkem_v_datech)

dle_veku_a_obce %>% ggplot(aes(x=velikost_obce, y = procent, color = zdroj, group = zdroj)) + geom_point() + geom_line() + facet_wrap(~kategorie,ncol = 1)
