...
if(! all(unique((data_sondy %>% filter(!is.na(kraj)))$kraj) %in% unique(data_skautis_2015$nazev_kraj))){
...
Pro zajímavost, takhle by to vypadalo, kdybychom vzali ze sond odpověď na otázku "Velikost obce, kde žiješ" - zejména u malých obcí to udělá dost rozdíl (asi dost lidí bydlí v malé obci, ale středisko mají ve větší). Je tedy důležité v dotazníku rozlišit obec kde žiji a obec kde mám středisko.

```{r}
seskupeni_obec_sondy_ziju <- data_sondy %>% 
  filter(!is.na(vek) & !is.na(velikost_obce)) %>%
  mutate(velikost_obce = factor(velikost_obce, levels = c("Do 1 999 obyvatel","2 000 – 9 999 obyvatel","10 000 – 99 999 obyvatel","100 000 a více obyvatel"), labels = c("< 2 000","< 10 000", "< 100 000","100 000 +"))) %>%
   rename(seskupeni = velikost_obce) 


reprezentativnost(seskupeni_obec_skautis_sondy, seskupeni_obec_sondy_ziju, "_sondy", "velikost_obce")
