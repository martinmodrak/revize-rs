...
clenstvi_pro_model <- clenstvi_pro_model_neseskupeno %>%
  ...
  ungroup() %>%
  mutate(vek_normalized = std_normalize(vek))

clenstvi_pro_model %>% ggplot(aes(x = vek, y = n_ukonceno/pocet, group = ID_Sex)) + geom_line(aes(color = ID_Sex)) + 
...
fit_ukonceno_vek_pohlavi <- brm(formula_ukonceno_vek_pohlavi, data = clenstvi_pro_model, file = here::here("local_data","fit_ukonceno_vek_pohlavi"))
...
```{r}
formula_ukonceno_vek_pohlavi_interaction <- brmsformula(n_ukonceno | trials(pocet) ~ s(vek_normalized, by = ID_Sex), family = "binomial")

#get_prior(formula_ukonceno_vek_pohlavi_interaction, data = clenstvi_pro_model)
priors = c(
...
fit_ukonceno_vek_pohlavi_interaction <- brm(formula_ukonceno_vek_pohlavi_interaction, data = clenstvi_pro_model, file = here::here("local_data","fit_ukonceno_vek_pohlavi_interaction"), prior = priors, control = list(adapt_delta = 0.95))

predicted <- fitted(fit_ukonceno_vek_pohlavi_interaction, newdata = data_for_prediction, nsamples = 1000, probs = c(0.025,0.25,0.75,0.975)) %>%
  cbind(data_for_prediction)  
...
clenstvi_pro_model_delka <- clenstvi_pro_model_neseskupeno %>%
  mutate(delka_clenstvi = Year - prvni_registrace) %>%
...
test_dlouholeti <- clenstvi_pro_model_delka %>% filter(vek >= 14, vek <=16, delka_clenstvi > 2)
run_pp_checks(fit_ukonceno_vek_pohlavi, test_dlouholeti)
```


```{r}
  
formula_ukonceno_vek_pohlavi_delka <- brmsformula(update.formula(formula_ukonceno_vek_pohlavi, . ~ . + delka_clenstvi_normalized), family = "binomial")

#get_prior(formula_ukonceno, data = clenstvi_pro_model)
fit_ukonceno_vek_pohlavi_delka <- brm(formula_ukonceno_vek_pohlavi_delka, data = clenstvi_pro_model_delka, file = here::here("local_data","fit_ukonceno_vek_pohlavi_delka"), control = list(adapt_delta = 0.95))
