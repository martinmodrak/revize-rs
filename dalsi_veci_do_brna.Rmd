...
```{r}
cela_data_rs %>% mutate(byl_na_kurzu = co_zazil %contains_word% "roversky_kurz") %>%
  group_by(byl_na_kurzu) %>% summarise(prumer_spokojenost = mean(spokojenost_s_roverskym_programem, na.rm = TRUE), sdev = sd(spokojenost_s_roverskym_programem, na.rm = TRUE)) %>%
  ggplot(aes(x = byl_na_kurzu, y = prumer_spokojenost, ymin = prumer_spokojenost - sdev, ymax = prumer_spokojenost + sdev)) +  geom_errorbar(color = "white") + geom_point(color = "white")
```
