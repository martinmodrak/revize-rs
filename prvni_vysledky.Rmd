...
#TODO sluzba ztraci label level
```


```{r}

test <- cela_data %>% rozsir_mc(quo(role_skauting))
test <- rozsir_vsechna_mc(cela_data)
```
...
```{r}
summarise_multiple_choice(cela_data, fungovani_skautskeho_oddilu) %>% arrange(desc(podil_ano))
```
