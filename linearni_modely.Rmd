...
```{r}
for(kategorie in unique(data_for_clm$kategorie_kompetence)) {
  #fits_clm[[kategorie]] <- clmm(kompetence_odpoved ~ 1 + age + kategorie_respondenta * kompetence + (1| session), data = data_for_clm %>% filter(kategorie_kompetence == kategorie))
  fits_clm[[kategorie]] %>% print()
}
```
