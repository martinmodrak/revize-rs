...
#inla_formula <- kompetence_relativne_k_populaci ~ 1 + age_norm + f(age_ar, model = "iid") + f(factor(kategorie_respondenta_full, model = "iid")) + f(mc_matrices$role_skauting, model = "z")
base_for_inla <- expanded_no_NA


pipeline_result <- inla_pipeline(base_for_inla, kategorie, inla_formula, kompetence_to_run = c("skautsky_zit"), save_cache = FALSE)
# pipeline_result <- inla_pipeline(base_for_inla, kategorie, inla_formula, kompetence_to_run = c("skautsky_zit"), save_cache = FALSE)
```
...
my_pp_check(pipeline_result,  role_skauting_clen_rady_roveru | 
                              ...
                              role_skauting_vedouci_roveru, label = "role_rover")
