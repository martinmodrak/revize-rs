...
#inla_formula <- kompetence_odpoved ~ 1 + age_norm + f(age_ar, model = "iid") + f(session, model="iid")
...
for(k in kompetence) {
  ...
  inla_fits[[k]] <- my_inla_fit(inla_formula, data = regression_inputs[[k]])
}
...
for(k in kompetence) {
  predicted_pp_checks[[k]] <- pp_samples_from_matrix(inla_formula, pp_samples_matrices[[k]], regression_inputs[[k]])
}
...
```{r}
my_pp_check(inla_formula, regression_inputs, predicted_pp_checks, age)
my_pp_check(inla_formula, regression_inputs, predicted_pp_checks, age, stat = sd)
my_pp_check(inla_formula, regression_inputs, predicted_pp_checks, kategorie_respondenta_full)

```

```{r}
my_pp_check(inla_formula, regression_inputs %>% purrr::map( ~ mutate(., byl_na_kurzu = co_zazil %contains_word% "roversky_kurz")), predicted_pp_checks, byl_na_kurzu)

```

```{r}
my_pp_check(inla_formula, regression_inputs %>% purrr::map( ~ mutate(.,  role_rover = role_skauting %contains_word% "clen_rady_roveru" | 
...
my_pp_check(inla_formula, regression_inputs %>% 
              purrr::map( ~ mutate(.,  
