...
inla_formula_base <- kompetence_odpoved ~ 1 + age_norm + f(age_ar, model = "iid") + f(kategorie_respondenta_full, model = "iid", hyper = list(theta = list(prior = log_inv_hn_sqrt(1)))) + f(row_id, model = "iid")


pipeline_result <- inla_pipeline(base_for_inla, kategorie, inla_formula_base, list(quo(role_skauting), quo(co_zazil), quo(fungovani_skautskeho_oddilu), quo(organizace_spolecenstvi)))
...
my_pp_check(pipeline_result, kategorie_respondenta_full) + theme(axis.text.x = element_text(angle = 90, vjust = 0.3, hjust = 1))
