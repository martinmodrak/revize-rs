...
for(kategorie in kategorie_kompetence) {
  pipeline_results[[kategorie]] <- inla_pipeline(base_for_inla, kategorie, inla_formula_base, mc_sloupce_k_pouziti)
}
...
for(pipeline_result in pipeline_results) {
  my_pp_check(pipeline_result, age, stat = sd) 5:
...
```{r}
for(pipeline_result in pipeline_results) {
  my_pp_check(pipeline_result, co_zazil %contains_word% "roversky_kurz") %>% print()
}
```

```{r}
for(pipeline_result in pipeline_results) {
  my_pp_check(pipeline_result, co_zazil %contains_word% "roversky_kurz"
                       | co_zazil %contains_word% "radcovsky_kurz"
                       | co_zazil %contains_word% "cekatelky"
                       | co_zazil %contains_word% "jiny_kurz"
                       | co_zazil %contains_word% "vudcovky", label = "jakykoliv_kurz") %>% print()
}
```
...
```
tt <- function(log_x, sigma) {  -1/(2 * sigma * log_x) - 0.5 * (log(2) + log(3.1415926535)) - log(sigma) - 1.5 * log_x }


limit <- 30
vals <- 1/(rnorm(1e3, sd = 100)^2)
curve(exp(tt(x, 1) + x), log(min(vals)),log(max(vals)))
dd <- density(log(vals))
plot(dd)
