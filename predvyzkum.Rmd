...
strediska <- strediska %>% 
  mutate(velikost_obce = cut(obyvatele, c(0,10000,100000,2000000), labels = c("do 10 tis","10-100 tis", "nad 100 tis")),
         pocet_roveru_category = cut(pocet_roveru, c(-1, quantile(pocet_roveru, probs = c(0.33,0.66)), 10000), labels = c("Malo", "Stredne", "Hodne")),
...
strediska_kategorie <- strediska %>%
  group_by(region, ma_kmen, velikost_obce, pocet_roveru_category) %>% summarize(pocet = length(ev_cislo)) %>%
  ungroup() %>%
...
strediska <- strediska %>% inner_join(strediska_kategorie, by = c("ma_kmen", "velikost_obce", "pocet_roveru_category", "region"))
```
...
for(group in unique(strediska_kategorie$skupina_id)) {
  ...
  strediska %>% filter(skupina_id == group) %>% sample_n(5) %>% select(ev_cislo, nazev_jednotky, pocet_roveru, ma_kmen, web) %>% kable("html") %>% kable_styling(bootstrap_options = "condensed") %>% print()
}
```
