...
mc_sloupce$zaklad <- c("role_skauting", "co_zazil", "fungovani_skautskeho_oddilu", "organizace_spolecenstvi")
dalsi_sloupce$zaklad <- ~ . 

mc_sloupce$hodne <- c(mc_sloupce$zaklad, "vyroky_o_roveringu_zazil", "vyroky_o_roveringu_zazil_2", "vychovne_nastroje", "problemy_roveringu")
  ...
  for(zakladni in c("zaklad", "hodne") ) {
    for(dalsi in c("minimal", "zaklad", "hodne")) {
      for(meritko in meritka_k_testovani) {
        ...
        print(formula)
        results <- inla_pipeline(base_for_inla, kategorie, formula, mc_sloupce[[dalsi]], n_cores = 7)
        for(kompetence in names(results$processed_fits)) {
          all_marginals_list[[length(all_marginals_list) + 1]] <- results$processed_fits[[kompetence]]$marginals_summary %>%
...
```{r}
all_mc_sloupce <- unique(do.call(c, mc_sloupce)) 
all_marginals_processed <- all_marginals
for(mc_sl in all_mc_sloupce) {
  marginal_pattern <- paste0("^id\\.", mc_sl,"(_NA|\\.[a-z_]*)?$")
  ...
  all_marginals_processed <- all_marginals_processed %>% 
    filter(!grepl(paste0(marginal_pattern), marginal)) %>%
    rbind(current_marginal_processed %>% select(-marginal_old, -expected_marginal, -index_int))
}
```

```{r}
unique(all_marginals_processed$marginal)
```
...
marginals_to_show <- all_marginals_processed %>% 
  ...
  group_by(marginal, kategorie, kompetence) %>%
    ...
    print(
      marginals_to_show %>%
        ...
        ggplot(aes(x = model, y = marginal, fill = widest_ci_sign)) +
          geom_tile() + theme(axis.text.x = element_text(angle = 90)) + facet_wrap(~kategorie)
    )
  }
#}
```
