...
settings <- tibble(N = c(500, 750, 1000)) %>% crossing(subset_questions =  c(11, 22)) %>% 
  crossing(N_fixed = c(4, 6), N_monotonic = c(4, 6), N_random_groups = list(base_random_groups, c(base_random_groups, 6, 6, 6, 5, 6, 8, 8, 6))) %>%
  mutate(N_monotonic_cat = 6, N_questions = 22, questions_correlation = TRUE, ncat= 7)
...
for(i in rev(1:nrow(settings))) {
  ...
  if(file.exists(result_file)) {
    sbc_results[[i]] <- result_file
  } else {
    params <- as.list(settings[i,])
    params$N_random_groups <- settings$N_random_groups[[i]]
    sbc_results[[i]] <- sbc(model, function() { do.call(simulate_data_reject, params)}, N_steps = sbc_steps, cores = 4, control = list(adapt_delta = 0.9))
  }
}
```
