...
Zároveň většina lidí, co v Junáku "přežije" do roverského věku Mimochodem tohle je v lajně s tím, že tím, jak chybí nováčci, tak zůstává málo starších ve skautských družinách a ti jsou pak rádci/podrádci. 
...
Jako singles bereme lidi bez společenství, i když sami sebe za rovera nepovažují.  
...
`scales::percent(podil_roveru_mezi_bez_spolecenstvi)` lidí v roverském věku, kteří jsou mimo společenství považuje "rover" za jednu ze svých rolí.
...
Nejvíc se ochází protože společenství zanikne nebo nefunguje, málokdy (`r scales::percent(podil_jen_zaniklo_nefungovalo)` to je ale jediný důvod.
...
```{r}
plots$spolecenstvi_registrace <- plot_summary_mc(hlavni_data, spolecenstvi_registrace, title = "Společenství a registrace")
...
plots$typ_id_strediska <- 
  plot_summary_mc(hlavni_data, typ_id_strediska, order_by_podil = FALSE, invert_color_threshold =  0.03)
plots$typ_id_strediska
```
