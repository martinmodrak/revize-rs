...
- To, že kmen má funkční organizaci koreluje s celou řadou pozitivních charakteristik. Naopak absence vedení/programu nebo roverského know-how je často zmiňovaným problémem.
...
- Roveři jsou v Junáku obecně velmi aktivní.

Detaily a zdůvodnění lze nalézt v příslušných částech dokumentu.

## Jak interpretovat tato data {#jak-interpretovat}

Pro interpretaci výsledků je potřeba připomenout, že všechny údaje jsou sebehodnocením a samozřejmě se lidé mohou lišit v tom, jak "přísní" na sebe a svá roverská společenství jsou. Nesmíme také zapomenout na možnou nepozornost nebo únavu při vyplňování. Proto zejména odpovědi, kde s něčím (ne)souhlasí jen malá procenta respondentů nelze brát příliš doslova. Je totiž velmi těžké napsat otázku, kde se  shodnou všichni. Například: jen 89% respondentů jednoho amerického průzkumu uvedlo, že "Férové volby jsou pro demokracii důležité".
...
## Vstup do roverského věku {#vstup_do_rs}
...
> Data neukazují masivní exodus začátkem roverského věku. Nízký počet roverů je pravděpodobně více ovlivněn výrazným poklesem přílivu nováčků s rostoucím věkem a celkově vysokým "odpadem" nováčků --- až 33% prvně registrovaných se neregistruje podruhé. Velká míra odchodů je ale vidět i u zkušenějších členů napříč skautským věkem --- každý rok odchází až 18% zkušenějších členů. Odchod člena v rovererském věku je také pro středisko závažnější, což může subjektivně vést ke zveličování počtu odchodů.

Totéž lze shrnout obrázkem:

![Podíl odchodů dle délky předchozí registrace a věku](shared_figs/odchody.png)

_Na obrázku vidíme, jaká část členské základny Junáka daného věku odešla během sledovaného období, rozděleno na nováčky (registrují se poprvé) a ostatní. Raději příklad: Kdyby za roky 2012---2018 bylo v Junáku celkem 1000 chlapců, kteří se poprvé registrovali během svých 13ti let a z toho se jich 200 již znovu neregistrovalo, bude v grafu 0.2. Pruh kolem údajů ukazuje očekávanou nejistotu/šum (čím méně lidí v kategorii, tím méně si jsme jisti, že to co vidíme je reprezentativní). Pozorovaný nárůst odchodů mužských členů s 2---3 letou zkušeností v roverském věku není příliš zásadní, protože absolutní počet těchto členů je nízký. _
...
### Čím si prošli před vstupem {#pred_vstupem}

Vícekrát jsme se setkali s domněnkou, že nefunkční družinový systém je příčinou problémů roveringu. Slušné množství roverů přichází z oddílů, kde družinový systém aspoň trochu fungoval, takže se na to nemůžeme tolik vymlouvat --- jakkoliv námi položené otázky nemohou přímo odhalit, jestli družiny skutečně fungovaly dobře. Odpovědi našich respondentů jsou ostatně velmi podobné výsledkům z nedávných [sond o družinovém systému](https://krizovatka.skaut.cz/balicky-ustredi/950-balicek-leden-2020/4907-vysledky-sond-k-tematu-druzinovy-system-v-oddilech).

```{r fungovani_skautskeho_oddilu}
plot_summary_mc(hlavni_data, fungovani_skautskeho_oddilu, "Moje družina ve skautském věku") 
...
```{r zazil_radce}
hlavni_data %>% plot_summary_mc(co_zazil, "Co zažili ve skautingu", subtitle = "jen otázky k rádcování", include_values = c("radce_podradce", "radcovsky_kurz"))

```

### Věk vstupu do roverských společenství {#vek_vstupu}

Zdá se, že málokdo vstupuje do kmene později než těsně po skautském věku.

```{r vek_vstupu_do_kmene, fig.cap='Rozložení věku vstupu do kmene. Věk vstupu je určen z otázek "Jak dlouho jsi ve společenství" a "Věk". Proto lze určit jen u těch, kteří byli v době průzkumu ve společenství.'}
...
plot_vek_vstupu_base +
  ggtitle("Věk vstupu do kmene", "Jen členi společenství")

#podil_vstup_17_nad_20_let <- 
  # data_vstupy %>% filter(!is.na(vek_vstupu_do_kmene), age == 20) %>%
  # summarise(podil = mean(vek_vstupu_do_kmene > 17)) %>% pull(podil)

```

Tato data jsou ale zkreslená směrem dolů tím, že obecně máme hodně mladších respondentů (16ti letý respondent nemohl vstoupit do kmene před 16tým rokem). Můžeme se tedy na stejný graf podívat separátně pro několik věkových kategorií: 

```{r vek_vstupu_do_kmene_skupiny}
plot_vek_vstupu_base + 
...
I i u respondentů ve věku 20 --- 23 let vidíme výraznou špičku vstupů mezi 15 a 17tým rokem. U ještě starších tato špička mizí. Tento pohled je ale zase zatížen chybou na druhou stranu. Ti, kterým je dnes 25 a do společenství vstoupili v 17ti už nejspíš ve společenství nejsou a tím pádem se nám nevejdou do tohoto pohledu, protože věk vstupu víme bohužel jen u současných členů společenství.


Hlavním důvodem, proč jsme přesvědčeni, že pozdější vstupy do roverského společenství jsou vzácné je pohled na vývoj v čase: Vidíme, že kolem 16tého roku jsou zhruba 3/4 respondentů ve společenství. S rostoucím věkem pak postupně klesá podíl roverů, co jsou ve společenství a srovnatelně přibývá "odpadlíků", zatímco kategorie "Nikdy nebyl ve společenství" se s věkem nijak výrazně nezmenšuje a zůstává kolem 20%. 

```{r kategorie_respondenta_v_case}
 hlavni_data %>% 
...
### Vlastní vstup {#vlastni-vstup}

Jak vypadá vlastní přechod do roverského věku? Tady je opět těžká interpretace: necelých 60% uvedlo, že mělo snadný přechod. Je to úspěch nebo se máme rmoutit pro dva z pěti roverů, co to dle vlastního pocitu snadné neměli? Obdobně, když si zhruba polovina roverů myslí, že byla připravena se aspoň trochu podílet na přípravě programu roverského společenství, je to hodně nebo málo?
...
```{r vyroky_zazil_vstup}
vyroky_vstup <- c("rover_automaticky","vstupni_ritual","snadny_prechod", "pri_prechodu_schopny")
plot_summary_mc(hlavni_data, vyroky_o_roveringu_zazil_2, "Rovering, který jsem zažil", subtitle = "jen otázky ke vstupu do roverského věku", include_values = vyroky_vstup)
...
```{r}
hlavni_data  %>% plot_binarni_s_nejistotou(paste0("vyroky_o_roveringu_zazil_2.",vyroky_vstup), by = age, facet = TRUE, na.rm = TRUE, names_prefix = "vyroky_o_roveringu_zazil_2.") + plot_annotation(paste0("Rovering, který jsem zažil"))
...
```{r vyroky_o_roveringu_stredisko_informace}
plot_summary_mc(hlavni_data, vyroky_o_roveringu_stredisko, "O roverech v mém středisku platí", subtitle = "(jen ti, co již nejsou v mladším oddíle a nikdy nebyli ve společenství)", include_values = c("je_spolecenstvi","pravidelne","bez_ulohy", "vlastni_akce", "nejsou", "nevim"))
...
## Život ve společenství {#zivot-ve-spolecenstvi}
...
```{r pocet_clenu_spolecenstvi}
plot_summary_mc(hlavni_data, pocet_clenu_spolecenstvi, order_by_podil = FALSE, invert_color_threshold =  0.03)
...
```{r vyroky_o_roveringu_zazil_zivot}
plot_summary_mc(hlavni_data, vyroky_o_roveringu_zazil, "Rovering, který jsem zažil", subtitle = "jen části týkající se života v kmeni", include_values = c("pro_jednu_gen", "koedukovany", "spoluprace_mimo_stredisko")) 


```

```{r vyroky_o_roveringu_zazil_2}
vyroky_zazil_obecne <- c("mladsi_starsi", "prechodovy_ritual", "jasne_ukonceni", "jak_dlouho_chci", "roversky_slib")
plot_summary_mc(hlavni_data, vyroky_o_roveringu_zazil_2, "Rovering, který jsem zažil 2", include_values = vyroky_zazil_obecne, subtitle = "jen části týkající se života v kmeni")
...
```{r vyroky_zazil_obecne_vek}
hlavni_data  %>% plot_binarni_s_nejistotou(paste0("vyroky_o_roveringu_zazil_2.",vyroky_zazil_obecne), by = age, facet = TRUE, na.rm = TRUE, names_prefix = "vyroky_o_roveringu_zazil_2.") + plot_annotation(paste0("Rovering, který jsem zažil 2"))


```


### Frekvence akcí {#frekvence-akci}
...
```{r frekvence_kratkych_akci}
plot_summary_mc(hlavni_data, frekvence_kratkych_akci, "Frekvence kratších akcí", order_by_podil = FALSE, invert_color_threshold = 0.03)


```

```{r frekvence_vicedennich_akci}
plot_summary_mc(hlavni_data, frekvence_vicedennich_akci, "Frekvence vícedenních akcí", order_by_podil = FALSE, invert_color_threshold = 0.03)


```
```{r frekvence_velkych_akci}
plot_summary_mc(hlavni_data, frekvence_velkych_akci, "Frekvence velkých akcí", order_by_podil = FALSE)
...
# 
# plot_summary_mc(hlavni_data, vyroky_o_roveringu_stredisko, "O roverech v mém středisku platí", subtitle = "(jen ti, co již nejsou v mladším oddíle a nikdy nebyli ve společenství)", include_values = c("je_spolecenstvi","pravidelne","vlastni_akce", "rozviji_se"))
# 
```


### Jak roversky žijí rádci? {#jak-roversky-ziji-radci}
...
## Nástroje roverského programu {#nastroje}
...
podil_kmen_alespon_jeden_klicovy <- sum(
  data_klicove_nastroje_shrnuto %>% filter(pocet_klicovych_nastroju >= 1) %>% pull(podil))
...
```{r vychovne_nastroje}
plot_summary_mc(hlavni_data, vychovne_nastroje, "Používáme tyto výchovné nástroje")
...
### Muži, ženy a nástroje {#muzi-zeny-nastroje}
...
Potvrzuje se  zkušenost z kurzů, že na ně častěji jezdí ženy --- a zde vidíme, že to nelze snadno svést na rozdílné počty roverů a rangers (ostatně dle registrace je v roverském věku v Junáku více mužů) --- společenství, kde jsou ženy nejspíš skutečně využívají roverské kurzy častěji.
...
```{r}
hlavni_data %>% plot_summary_mc(co_zazil, "Co zažili ve skautingu", exclude_values = c("radce_podradce", "radcovsky_kurz", "__nic"))
...
### Velké akce a neorganizované společenství {#velke-akce-a-neorganizovane-spolecenstvi}
...
To můžeme asi posoudit jen z toho, jak velká část takových společenství velké akce využívá. Jediný větší rozdíl je mezi neaktivními kmeny o ostatními, jinak velký rozdíl nevidíme. Jak moc jsou velké akce "důležité" z toho samozřejmě zjistit nemůžeme. 

```{r}
plot_velke_akce_organizovan <- hlavni_data %>% plot_binarni_s_nejistotou("vychovne_nastroje.velke_akce", organizace_nejvyssi, na.rm = TRUE) + plot_annotation("Podíl kmenů užívajících velké akce", "dle různých měřítek organizace")

plot_velke_akce_organizovan
```

## Spokojenost roverů {#spokojenost}


```{r}
hlavni_data %>% plot_summary_mc(spokojenost_clenstvim_v_rs, "Spokojenost s členstvím ve společenství", 'Přesná otázka: "Se svým členstvím v roverském společenství jsem celkově spokojena"')
...
# plot_summary_mc(hlavni_data, s_cim_spokojen, title = "S čím jsi ve společenství spokojen", exclude_values = "postoj_strediska")
# 
# 
# 
# 
# plot_summary_mc(hlavni_data, s_cim_nespokojen, title = "S čím jsi ve společenství NEspokojen", exclude_values = "postoj_strediska") 
# 
...
## Stárnutí rovera {#starnuti-rovera}
...
# hlavni_data %>% filter(!is.na(let_v_kmeni)) %>% ggplot(aes( x = let_v_kmeni)) + geom_histogram(binwidth = 1) + ggtitle("Počet let ve společenství") + scale_x_continuous("Let ve společenství") + scale_y_continuous("Počet") + vodorovne_popisky_x 
# 
```
```{r}
...
Zároveň si myslíme, že průzkum je co se týká věku respondentů jakž takž reprezentativní (viz [Kapitola \@ref(reprezentativnost-vek)](#reprezentativnost-vek)). A --- jak [vidíme v registraci](#odchody-roveri) --- odcházení lidí z Junáka se sice po 15tém roku zpomaluje, ale stále zůstává nezanedbatelné, zatímco počet nováčků je v tomto věku minimální.  
...
hlavni_data %>% filter(kategorie_respondenta == "nyni_spolecenstvi") %>% plot_binarni_s_nejistotou(nazvy_problemy, by = age, names_prefix = "problemy_roveringu.", facet = TRUE) + plot_annotation(paste0("Problémy roveringu dle věku "), "pouze současní členové společenství")


```

## Odchod ze společenství {#odchod_z_rs}
...
```{r}
plot_summary_mc(hlavni_data, proc_neni_rover, "Proč už nejsem v roverském společenství")
...
Podobné výsledky nám dá, když se podíváme na to, co vidí respondenti jako hlavní problémy roveringu --- stěhování pryč je opět velmi výrazné, hned za přílišným vytížením v oddíle (vytížení v oddíle jsme bohužel opomněli dát jako možnost do otázky "Proč už nejsem rover"). Častým problémem je také absence vedení/programu.

```{r}
hlavni_data %>% plot_summary_mc(problemy_roveringu, "Hlavní problémy roveringu.", subtitle = "jen členi společenství", include_values = c("vytizeni_oddily","odstehovani","vytizeni_mimo", "nevidi_smysl", "spatne_vztahy", "bez_vedeni", "soupereni_mezi_skupinami"))
...
## Forma vedení a forma společenství {#vedeni_rs}
...
```{r}
plot_summary_mc(hlavni_data, organizace_spolecenstvi, title = "Organizace kmene")
...
# hlavni_data %>% filter(kategorie_respondenta != "nikdy_spolecenstvi") %>% plot_summary_mc(role_skauting, "Role ve skautingu", subtitle = "pouze kdo byl/je ve společenství, pouze roverské role", include_values = roverske_role, exclude_values = "rover_sam")
# 
...
Pro další kroky si strukturu organizace trochu zjednoduššíme a vezmeme "nejvyšší přítomnou organizaci" --- tj. tam kde mají vůdce/tahouna řadíme pod "ma_vudce", kde nemají vůdce ale mají radu, zařadíme pod "ma_radu" --- kde nemají ani, jedno, ale uvedli, že organizují všichni, necháme "vsichni" --- poslední dvě možnosti jsou, že neorganizuje nikdo, nebo že kmen není aktivní. To nám změní rozložení ve prospěch vůdců (protože jakákoliv zmínka o vůdci zařadí kmen do kategorie "má vůdce").


```{r}
hlavni_data %>% group_by(organizace_nejvyssi) %>% summarise(pocet = n(), .groups = "drop") %>%
  filter(!is.na(organizace_nejvyssi)) %>%
  mutate(podil = scales::percent(pocet / sum(pocet))) %>% table_format()

podil_organizace_nejvyssi_vsichni <- mean(hlavni_data$organizace_nejvyssi == "vsichni", na.rm = TRUE)
```

```{r}
# hlavni_data %>% filter(!is.na(organizace_nejvyssi)) %>%
#   mutate(neaktivni = organizace_nejvyssi == "neaktivni",
#          neni_organizovan = organizace_nejvyssi == "neni_organizovan",
#          organizuji_vsichni = organizace_nejvyssi == "vsichni",
#          ma_radu = organizace_nejvyssi == "ma_radu",
#          ma_vudce = organizace_nejvyssi == "ma_vudce") %>%
#   plot_binarni_s_nejistotou(c("neaktivni", "neni_organizovan", "organizuji_vsichni", "ma_radu", "ma_vudce"), age)
```



Zajímavé je, že přítomnost nějaké formy organizace je spojena jen se zanedbatelnou změnou toho, jak často respondenti uvádějí různé problémy. Jediný problém, který skutečně výrazně mizí s přítomností vedení je "Nikdo rovery neorganizuje/nevede", ale vidíme též nějaké rozdíly u "Pro rovery není kvalitní program" či "Nevíme jak na to". 

```{r}
hlavni_data %>% filter(kategorie_respondenta != "nikdy_spolecenstvi") %>%
  plot_binarni_s_nejistotou(nazvy_problemy, organizace_nejvyssi, flip = TRUE, names_prefix = "problemy_roveringu.") + svisle_popisky_x + plot_annotation("Problémy roveringu", "podle formy vedení")
...
V sondách do roveringu 2015 jsme našli asociaci mezi tím, že kmen má vůdce a větší spokojeností i větší aktivitou kmene. V současných datech vidíme větší spokojenost u alespoň nějak organizovaných kmenů a mírně nejvyšší je tam, kde funguje organizace všemi. Kmeny s vůdcem, radou nebo s organizací všemi mají také nejvyšší frekvenci akcí.

```{r}
hlavni_data %>% filter(kategorie_respondenta == "nyni_spolecenstvi") %>% plot_ciselne_s_nejistotou("spokojenost_clenstvim_v_rs", by = organizace_nejvyssi) + plot_annotation("Spokojenost s členstvím v RS", "dle organizace kmene, jen současní členové")
```

```{r}
hlavni_data %>% filter(kategorie_respondenta == "nyni_spolecenstvi") %>%plot_frekvence_by("frekvence_vicedennich_akci", organizace_nejvyssi) + plot_annotation("Frekvence vícedenních akcí", "dle organizace kmene, jen současní členové")
...
```{r}
hlavni_data %>% filter(!is.na(vychovne_nastroje), kategorie_respondenta == "nyni_spolecenstvi") %>% plot_ciselne_s_nejistotou("pocet_klicovych_nastroju", organizace_nejvyssi) + plot_annotation("Výchovné nástroje dle organizace", subtitle =  paste0("Počet použitých klíčových nástrojů. Za klíčové se berou:\n ", paste0(klicove_nastroje_id, collapse = ", ")))


```


Potenciálním problémem těchto porovnání je, že část společenství, které mají vůdce je v průzkumu reprezentována přímo těmito vůdci. Můžeme očekávat, že vůdci vnímají své společenství trochu jinak než ostatní členové. U porovnání frekvence akcí to ale nehraje velkou roli (graf vypadá téměř stejně, když odpovědi samotných vůdců vynecháme). U počtu použitých nástrojů to výsledky ovlivní (křivka napravo od "vsichni" již zůstává v podstatě rovná) --- může to tedy být tím, že vůdci mají celkově lepší znalost nástrojů, ač jejich společenství ve skutečnosti o tolik více nástrojů nepoužívají.

Spojitostí mezi organizací společenství a naplněním kompetencí se pak zabýváme i v kapitole \@ref(kompetenceasociace) o asociaci kompetencí s dalšími charakteristikami.

Celkově můžeme říct, že je hlavní, že je kmen nějak organizován. Jsou důvody považovat z dat za lepší když kmen má vůdce (spojeno s větší frekvencí akcí, více nástrojů programu), zároveň ale není jisté, že to není problém dat a že se to skutečně promítne v kompetencích či dalších zásadních výstupech. Pokud se tedy podaří kolektivní organizace, je dobré ji podpořit. Zároveň je ale podle nás určený vůdce --- pokud zároveň umí dát dost prostoru ostatním --- nejspolehlivější způsob organizace. Všimněme si též, že organizace "všemi" se jen málokdy vyskytuje bez zmínky o vůdci či radě (jen `r scales::percent(podil_organizace_nejvyssi_vsichni)`), takže v praxi není k vidění příliš často, z čehož lze usuzovat, že je těžké takové společenství rozjet --- což potvrzuje i naše osobní zkušenost. 

## Roveři ve vedení oddílu {#vedenioddilu}
...
```{r}
hlavni_data %>% plot_summary_mc(spokojenost_s_fungovanim_v_junaku, "Spokojenost s fungováním v Junáku", subtitle = "Jen bez roverske zkušenosti", invert_color_threshold = 0.05)
...
```{r}
plot_summary_mc(hlavni_data, proc_nebyl_rover, "Proč jsi nikdy nebyla\nsoučástí roverského společenství")
...
```{r}
hlavni_data %>% filter(!je_rover) %>% plot_summary_mc( bez_zkusenosti_velke_akce, "Účast na akcích", subtitle = "Nepovažuje se za rovera, bez roverské zkušenosti")


hlavni_data %>% filter(je_rover) %>% plot_summary_mc(bez_zkusenosti_velke_akce, "Účast na akcích", subtitle = "Jen bez roverské zkušenosti ale považuje se za rovera")
...
```{r}
plot_summary_mc(hlavni_data, bez_zkusenosti_setkavam_se_s_vrstevniky, "Setkávání s vrstevníky ", subtitle = "Jen bez roverské zkušenosti", order_by_podil = FALSE, invert_color_threshold = 0.02)

```

```{r}
plot_summary_mc(hlavni_data, bez_zkusenosti_seberozvojovy_program, "Seberozvojový program", subtitle = "Jen bez roverské zkušenosti", order_by_podil = FALSE, invert_color_threshold = 0.02)
...
```{r}
hlavni_data %>% plot_summary_mc(problemy_roveringu_stredisko, "Hlavní problémy roveringu", "Jak vidí své středisko ti, co nikdy nebyli ve společenství")
...
```{r}
plot_summary_mc(hlavni_data, vyroky_o_roveringu_zazil, "Rovering, který jsem zažil", subtitle = "jen části týkající se střediska", include_values = c("vedeni_je_rovering", "rovering_na_SR", "podpora_strediska")) 


```

```{r}
podil_setkavani_pomaha <- mean(hlavni_data$co_pomaha_roveringu.neformalni_setkani, na.rm = TRUE)
```


Zároveň ale finanční pomoc střediska nebo střediskový zpravodaj pro rovery nejsou často zmiňovány jako něco, co rovering pomáhá v porovnání např. se sociálními sítěmi, ale zase je to zmiňováno častěji než roverký program vydaný Junákem nebo celkově "vyšší místa" (nejčastěji zmiňované je neformální setkávání roverů se `r scales::percent(podil_setkavani_pomaha)` ).



```{r}
plot_summary_mc(hlavni_data, co_pomaha_roveringu, "Roveringu dle mé vlastní\nzkušenosti znatelně pomáhá:", subtitle = "zaostřeno na středisko", include_values = c("zpravodal","financni_podpora", "roversky_program", "socialni_site"))

```

Absence podpory střediskem je zmiňována jen poměrně vzácně. Necelá třetina roverů si pak myslí, že je středisko považuje za pracovní četu a je to problém.

```{r}
hlavni_data %>% plot_summary_mc(problemy_roveringu, "Hlavní problémy roveringu", subtitle = "jen věci týkající se střediska, jen členi společenství", include_values = c("odstehovani", "vytizeni_oddily", "bez_podpory_strediska", "pracovni_ceta", "soupereni_mezi_skupinami"))


```

## Kontakt s "vyšší" organizací {#kontakt-s-vyssi-organizaci}
...
Registrace nám bohužel nemůže dát dobrý pohled na roverská společenství --- jen třetina roverů uvádí, že jejich společenství je registrováno jako roverský kmen a většina členů je registrována u dětských oddílů. Nehledě na to, že v oddílech se běžně nesprávně přiřazují všechny věkové kategorie a roveři nejčastě (dle zkušeností Roverského odboru).


```{r}
plot_summary_mc(hlavni_data, spolecenstvi_registrace, title = "Společenství a registrace", subtitle = "jen roveři nyní ve společenství")
...
```{r}
hlavni_data %>% plot_summary_mc(komunikacni_kanaly_existujici, "Kde bys nejraději sledoval\n informace o roveringu")

hlavni_data %>% plot_summary_mc(komunikacni_kanaly_hypoteticke, "Kde bys nejraději sledoval\n informace o roveringu (hypotetické)")
...
hlavni_data %>% plot_binarni_s_nejistotou(nazvy_komunikace, by = age, names_prefix = "komunikacni_kanaly_existujici.", na.rm = TRUE, facet = TRUE) + plot_annotation(paste0("Komunikační kanály dle věku"), "Kde bys nejraději sledovala aktuální informace o roveringu?")

```



## Přínosy roveringu organizaci {#prinosy-roveringu-organizaci}
...
```{r}
hlavni_data %>% plot_summary_mc(role_skauting, "Role ve skautingu", subtitle = "mimo roverských rolí", exclude_values = c(roverske_role,  "__nic"))
...
```{r}
plot_summary_mc(hlavni_data, vyroky_o_roveringu_stredisko, "O roverech v mém středisku platí", subtitle = "(jen ti, co již nejsou v mladším oddíle a nikdy nebyli ve společenství)", include_values = c("pomocna_sila","roverske_heslo","vedou","akce_pro_druhe"))
...
```{r}
plot_summary_mc(hlavni_data, sluzba, title = "Služba za poslední měsíc") 
