...
Rozdělení služeb se příliš neliší když se podíváma zvlášť na rovery ve společenství a rovery mimo. Nejvyšší úroveň je u roverů, kteří nikdy nebyli ve společenství a jsou ještě součástí oddílu mladších členů - lze očekávat, že to je primárně rozdíl v sebehodnocení než skutečně výrazně vyšší aktivita.
...
Jsou problémy roveringu způsobeny tím, že nefunguje družinový systém? Dle našeho odhadu nejvýše část - slušné množství roverů přichází z oddílů, kde aspoň trochu fungoval.
...
```{r}
plots$zazil_radce <- hlavni_data %>% plot_summary_mc(co_zazil, "Co zažili ve skautingu", include_values = c("radce_podradce", "radcovsky_kurz"))
plots$zazil_radce
```


```{r}
plot_summary_mc(hlavni_data, vyroky_o_roveringu_zazil_2, "")
```
...
```{r}
plots$vyroky_o_roveringu_stredisko_informace <- plot_summary_mc(hlavni_data, vyroky_o_roveringu_stredisko, "O roverech v mém středisku platí", subtitle = "(jen ti, co již nejsou v mladším oddíle a nikdy nebyli ve společenství)", include_values = c("je_spolecenstvi","pravidelne","bez_ulohy","nejsou", "nevim"))
...
Z lidí v roverském věku, kteří nikdy nebyli ve společenství jich `r scales::percent(podil_neroveru_bez_informaci)` označilo, že o roverech na středisku nemá informace.
...
# 
# 
# hlavni_data %>% plot_frekvence_by(quo(frekvence_kratkych_akci), byl_na_kurzu)
# hlavni_data %>% plot_frekvence_by(quo(frekvence_vicedennich_akci), byl_na_kurzu)
# hlavni_data %>% plot_frekvence_by(quo(frekvence_velkych_akci), byl_na_kurzu)
...
plot_frekvence_by <- function(cela_data, nazev_freknce_sloupce, group) {
  ...
  names_prefix <- paste0(nazev_freknce_sloupce, "_")
  frekvence_names <- gsub(names_prefix, "", colnames(frekvence_cummulative), fixed = TRUE)
  ...
  frekvence_cummulative %>% cbind(vyplnena_data %>% select({{group}})) %>%
    ...
    group_by({{group}}, frekvence) %>% 
    summarise(prumer_ano = mean(frekvence_ano)) %>%
      ggplot(aes(x = frekvence, y = prumer_ano, color = {{group}}, group = {{group}}, shape = {{group}})) +  geom_line() + geom_point() + scale_x_discrete(nazev_freknce_sloupce)
}
...
```{r}
# Roverský program vydaný Junákem
...
Spokojenost s členstvím _velmi mírně_ klesá s věkem, těžko z toho dělat silné závěry, protože, jak si za chvíli ukážeme, s věkem klesá frekvence akcí a frekvence akcí taky koreluje se spokojeností, takže velká otázka, co je zde příčinou a následkem.
...
```{r}
plots$co_zazil <- hlavni_data %>% plot_summary_mc(co_zazil, "Co zažili ve skautingu", exclude_values = c("radce_podradce", "radcovsky_kurz"))
plots$co_zazil
```
