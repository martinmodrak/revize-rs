...
effects_by_prediction <- function(fit, orig_data, var_name, var_values, var_labels = var_values, response_title, num_low_categories, n_samples = 500, max_lines = 200) {
  ...
  orig_data_for_plot <- orig_data %>% 
    ...
    group_by(var_value) %>%
    summarise(p_vetsi = 1 - mean(celkova_spokojenost_int <= num_low_categories),
              pocet = length(celkova_spokojenost_int))
...
recode_frekvence <- function(x) {
  ...
  recode(x, .missing = 1, `Nikdy` = 1, `Méně často než 1 za rok` = 2, `Asi jednou za půl roku` = 3, `Asi jednou za čtvrt roku` = 4, `Asi jednou měsíčně` = 5, `Asi jednou za 14 dní` = 6, `Každý týden nebo častěji` = 7) %>% as.integer() 
}
