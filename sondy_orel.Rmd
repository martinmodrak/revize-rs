...
recode_frekvence <- function(x) {
  recode(x, .default = NA_real_, `Nikdy` = 1, `Méně často než 1 za rok` = 2, `Asi jednou za půl roku` = 3, `Asi jednou za čtvrt roku` = 4, `Asi jednou měsíčně` = 5, `Asi jednou za 14 dní` = 6, `Každý týden nebo častěji` = 7) %>% as.integer() 
}
...
fit_spokojenost <- brm_multiple(formula_spokojenost, data = spokojenost_imputed, prior = priors, control = list(adapt_delta = 0.95), file = here("local_data","fit_spokojenost"))
fit_spokojenost
...
priors_multivar <- c("frekvence_rs_akci_celodenni_akce","frekvence_rs_akci_schuzky","frekvence_rs_akci_vicedenni_akce", "frekvence_rs_akci_tabory_expedice") %>%
  gsub("_","", ., fixed = TRUE) %>%
...
fit_aktivita <- brm_multiple(formula_aktivita, data = spokojenost_imputed, prior = priors_multivar, control = list(adapt_delta = 0.95), family = cumulative("logit"), file = here("local_data","fit_aktivita")) 
fit_aktivita
