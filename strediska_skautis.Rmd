...
datasety_wide$pouzitelne %>% 
  select(reg_c_strediska, nazev_strediska) %>%
...
```{r}
nesparovana <- datasety_wide$pouzitelne %>% filter(typ_id_strediska == "reg_cislo", !(str_trim(reg_c_strediska) %in% strediska_skautis_id$RegistrationNumber)) %>% select(reg_c_strediska, nazev_strediska, nazev_oddilu)
